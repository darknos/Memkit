      ...
      case "read":
        ...
        apiCall(params, function(_response) {
            if (_response.success) {
                var data = parseJSON(DEBUG, _response, parentNode);
                var values = [];
                params.add || (model.length = 0);
                if (_.isArray(data)) for (var i in data) {
                    var item = {};
                    item = data[i];
                    void 0 == item[model.idAttribute] && (item[model.idAttribute] = guid());
                    values.push(item);
                    params.add || model.length++;
                    params.success(1 === data.length ? values[0] : values, _response.responseText);
                } else {
                    values.push(data);
                    model.length = 1;
                    params.success(values[0], _response.responseText);
                }
                model.trigger("fetch");
            } else {
                params.error(JSON.parse(_response.responseText), _response.responseText);
...
function logger(DEBUG, message) {
    DEBUG && Ti.API.debug("[SQL REST API] " + message);
}

function parseJSON(DEBUG, _response, parentNode) {
    logger(DEBUG, "server response", _response.responseText);
    var data;
    try {
        data = JSON.parse(_response.responseText);
    } catch (e) {
        Ti.API.debug("JSON.parse failed");
    }
    if (!data) try {
        data = eval("(" + _response.responseText + ")");
    } catch (e) {
        Ti.API.debug("eval failed");
    }
    if (!data) {
        alert(L("internet_error"));
        return {};
    }
    _.isUndefined(parentNode) || (data = traverseProperties(data, parentNode));
    return data;
}
