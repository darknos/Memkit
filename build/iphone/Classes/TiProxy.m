...
//Rare exceptions to indicate a bug in the _mignews code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_mignews = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_mignews]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.mignews.module"
